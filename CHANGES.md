...
## Release 4.1.0
* Updating `SSH.sftp_get` and `SSH.sftp_put` methods to ensure sftp transport is closed regardless of whether an exception occurs. [d7a1ced]
...
* Moved `do_shell_script` to `shell_command` to match `ssh_command` from SSH class.  `do_shell_script` will be removed in sshreader v4.0 [de4523d]
...
* Added connect kwarg to SSH class and testing for established connection when issuing ssh_command. [35f5d24]
* Silencing `progress_bar` when used in conjunction with debuglevel. [dab6ac2]
...
* Added `return_code` to ShellCommand namedtuple. [182570b]
...
* Renamed `ServerJob.ssh_con` to `ServerJob._conn` [c4fe4b5]
* Added FAQ to docs for including helpful hints. [f7139c6]

## Release 2.3.0
* Importing `__future__` statements for `print_function` and division
